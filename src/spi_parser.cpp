...
void SPIParser::Communicate(SPIDataStruct *Data) {
    ...
    this->LastSensorDataUpdate = esp_timer_get_time(); //force update so we can get new data
    ...
    if(Data && (this->LastSliderPos != Data->SliderPos)) {
        Serial.println("if(Data && (this->LastSliderPos != Data->SliderPos)) {");
        this->LastSliderPos = Data->SliderPos;
        if(LEDHandler) {
            Serial.println("LEDHandler->SetLEDBrightness(0);");
