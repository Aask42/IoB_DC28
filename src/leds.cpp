                ...
                case 3: {
                    ...
                    for(i = 0; i < 2; i++, tempdata <<= 2) {
                        switch((tempdata >> 2) & 0x3) {
                            ...
                            case 1:
                                //doing nothing
                                Serial.println("leds.cpp, switch((tempdata >> 2) & 0x3) { case 1:, does nothing");
                            case 2:
                                //doing nothing
                                Serial.println("leds.cpp, switch((tempdata >> 2) & 0x3) { case 2: , does nothing");
                            case 3:
                                yy2 = (data & 0x30) >> 4;
                    ...
                    yy2 = 0; //make sure output is not shifted
                }
            }
            break;
        case 2:
...
void LEDsInternal::Run() {
    ...
    while(1) { //for always, just do the thing
        pthread_mutex_lock(&led_lock); //lock the mutex so that we avoid anyone modifying the data while we need it
        ...
        while(cur_script) {
            if(!cur_script->active) { //if not active skip it
            ...
            while(!cur_script->DelayEndTime && !LogicSeen) {
                ...
                cur_script->active_pos++;
                if(!(Command & 0x80)) { //top bit indicates math or other command
                    //math operations, get all of our fields
                    //and determine the math operation to perform
                    yy = (Command & 0x30) >> 4;
                    ...
                    if(!((DestData & 0x30) >> 4)) {
                        ...
                        if(yy) {
                            cur_script->leds[ID].Time[yy - 1] = 0; //reset the times according to what destination we are modifying if a led
                        }
                        else {
                    ...
                    if((Command & 0xf) != 0xa) { //if "not" then ignore value
                        Value = this->GetValue(cur_script, Command & 0xf, DestData, yy);
                    }
                    else {
                        Value = *CurValPtr;
                    }
                    if(yy) { //determine the masking based on dest
                        Mask = 0xff << (24 - (yy*8));
                    }
                    else {
                        Mask = 0x00ffffff;
                    }
                    DestMask = 0xffffff & ~Mask; //generate the destination mask
                    switch(Command & 0xf) { //look up the math operation to perform
                        case 0x0: //set
                        ...
                        case 0x1: //move
                            ...
                            //go alter the LED info, what could go wrong?
                    ...
                    switch(Command & 0x7) {
                        ...
                        case 0x2: //if
                            //doing nothing
                            Serial.println("leds.cpp, switch(Command & 0x7) { case 0x2:, does nothing");
                        case 0x3: { //wait
                            ...
                            uint32_t CompareValue = this->GetValue(cur_script, 0, DestData, yy2); //get the value
                            ...
                            LogicSeen = 1; //indicate we saw logic
                            break;
                        }
        ...
        for(i = 0; i < LED_Count; i++) {
            ...
            if(this->leds[i].OverrideTime) { //see if we need to adjust over time
                ...
                if(!cur_script) { //if no script then continue
                    continue;
                }
...
LEDsInternal::ScriptInfoStruct *LEDsInternal::FindScriptForLED(uint16_t LED) {
    ...
    while(cur_script) {
        ...
        if(cur_script->active && (cur_script->LEDMask & (1 << LED))) {
            ...
            return cur_script; //found a script impacting this led, stop looking
        }
        cur_script = cur_script->next;
    }
    return 0; //return that we couldn't find anything
}
...
void LEDsInternal::SetLED(ScriptInfoStruct *cur_script, uint8_t entry) {
    ...
    if(this->leds[entry].Enabled || this->leds[entry].OverrideTime || (cur_script && cur_script->TempOverride && (cur_script->LEDMask & (1 << entry)))) {
...
void LEDsInternal::AddToStack(ScriptInfoStruct *script) {
    ...
    while(CurScript) {
        ...
        CurScript = CurScript->next; //next one
    }
}
...
void LEDsInternal::RemoveFromStack(ScriptInfoStruct *script) {
    ...
    if(script->stackfirst == script) { //if we are the first entry then update the whole chain
        ...
        if(!NewHead->stacknext) { //if the new head does not have any entries then reset it's stackfirst
            NewHead->stackfirst = 0;
        }
        else {
            StackScript = NewHead;
            while(StackScript) { //update the chain for who is first
...
void LEDsInternal::ReadAmbientSensor() {
    //TODO Known Break: this is all sorts of broke. We have to power down wifi and reset registers to fix this
    this->LastAmbientReading = esp_timer_get_time(); //store the last time we got the sensor
    return;
}
...
void LEDsInternal::SetLEDBrightness(float BrightnessPercent) {
    ...
    this->MaxBrightness = int(pow(pow(MAX_RESOLUTION, POWER_LAW_A) * BrightnessPercent, 1/POWER_LAW_A) + 0.5); //calculate a new value for max
#endif

    for(i = 0; i < LEDEnum::LED_Count; i++) { //force all LEDs to set their brightness accordingly
...
#if BOI_VERSION == 1
    return this->MaxBrightnessPercent; //return the percent set
#elif BOI_VERSION == 2
...
#define MAX_SET_LED LED_8
    ...
    for(i = MIN_SET_LED; i <= MAX_SET_LED; i++) { //enable all LEDs
        this->leds[i].Enabled = 1;
    }
...
#elif BOI_VERSION == 2
    for(i = MAX_SET_LED - Count; i >= MIN_SET_LED; i--) //led_0 is the top light so work backwards
#endif
    {
    ...
    for(i = MIN_SET_LED; i <= MAX_SET_LED; i++) { //force all refreshed
        this->SetLED(this->FindScriptForLED(i), i);
    }
